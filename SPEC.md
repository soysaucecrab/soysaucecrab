...
## v1.1 방향 전환 — "컴퓨터/에디터" UI (2026-09-12)

- 메인 UI를 **에디터형(IDE) 앱**으로 전환. OS UI를 빌려 정체성을 강화:
  데스크톱(`/`) = VS Code식 IDE(탐색기 트리 · 탭 · 에디터 · 하단 미니
  셸 터미널 · ⌘K/⌘P 팔레트 · 상태바), 모바일(`/m`) = iOS Files식
  네비게이션. 폭 720px 기준으로 `/`↔`/m` 자동 리다이렉트.
- 터미널: `ls/cd/cat/open/tree/whoami/theme/lang/clear/help` + Tab 완성,
  명령 히스토리, 블록 커서(포커스 시 깜빡). 클릭 내비의 덤(점진적 향상).
- **ESC**: 팔레트 → 터미널 → 열린 탭 순으로 닫음.
- 폰트는 그대로 self-host Ubuntu / Ubuntu Mono / Noto Sans KR.
- 콘텐츠는 현재 앱에 인라인(작음). 추후 Content Collections에서
  생성하도록 배선 예정. 기존 단일컬럼 라우트(`/news` 등)는 제거.
- 후속: 실제 URL 딥링크, OG 이미지, a11y 보강, VS Code/Finder 통짜
  클론은 이스터에그로.
